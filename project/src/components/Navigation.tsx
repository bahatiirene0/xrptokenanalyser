...
import { Menu, X, BarChart3, FileText, Target, Stethoscope, DollarSign, Map } from 'lucide-react';
...
const Navigation: React.FC = () => {
  ...
  const navItems = [
    ...
    { name: 'Purpose', icon: Target, href: '#purpose' },
    { name: 'Tokenomics', icon: DollarSign, href: '#tokenomics' },
    { name: 'Roadmap', icon: Map, href: '#roadmap' },
    { name: 'Whitepaper', icon: FileText, href: '#whitepaper' },
    { name: 'TokenDoctor', icon: Stethoscope, href: '#tokendoctor' },
  ];
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="group flex items-center space-x-2 text-gray-300 hover:text-green-400 px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 hover:bg-green-500/10"
                >
